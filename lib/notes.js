...
const createNewNotes = (body, notesArray) => {
    ...
    try {
        ...
        console.log('✔️ ' + `${getCurrentDateandTime()} | ${note.title} has been ADDED!`);
...
const deleteNote = (id, notesArray) => {
    ...
    console.log('🔥 ' + `${getCurrentDateandTime()} | ${notesArray[locateNoteIndex].title} has been DELETED!`)
...
const updateNote = (id, updatedData, notesArray) => {
    // Locate the index of the note within it's array
    let locateNoteIndex = notesArray.findIndex(note => note.id === id);

    // Update the note's title and text with the new data 
    notesArray[locateNoteIndex].title = updatedData.title || notesArray[locateNoteIndex].title
    notesArray[locateNoteIndex].text = updatedData.text || notesArray[locateNoteIndex].text
    ...
    try {
        ...
        console.log('✔️ ' + `${getCurrentDateandTime()} | Note successfully updated`);

        // return finished code to post route for response
        return notesArray[locateNoteIndex];
    } catch (error) {
        console.log('Something went wrong with createNewNotes writeFileSync:' + error);
    }
}
...
// Export the methods
export { findByID, createNewNotes, validateNote, deleteNote, updateNote};